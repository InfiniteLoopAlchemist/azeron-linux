#!/bin/bash
# .deb postinst hook — install Azeron udev rules and reload udev.
#
# The .deb ships the udev rules at /opt/azeron-software/udev/99-azeron.rules
# but they need to be at /etc/udev/rules.d/99-azeron.rules for the kernel
# to apply them. Without this hook, users hit "cannot open device with path
